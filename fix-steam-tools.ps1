...
# Step 2: Check if hid.dll or xinput1_4.dll exists in Steam directory
Write-Host "`n[Step 2] Checking if steamtools is installed..." -ForegroundColor Yellow

$hidDllPath = Join-Path $steamPath "hid.dll"
$xinputDllPath = Join-Path $steamPath "xinput1_4.dll"

$hidExists = Test-Path $hidDllPath
$xinputExists = Test-Path $xinputDllPath

if ($hidExists -or $xinputExists) {
    if ($hidExists) {
        Write-Host "hid.dll found at: $hidDllPath" -ForegroundColor Green
    }
    if ($xinputExists) {
        Write-Host "xinput1_4.dll found at: $xinputDllPath" -ForegroundColor Green
    }
} else {
    Write-Host "hid.dll NOT found at: $hidDllPath" -ForegroundColor Red
    Write-Host "xinput1_4.dll NOT found at: $xinputDllPath" -ForegroundColor Red
